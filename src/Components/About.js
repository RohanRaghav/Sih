import React from 'react'

const About = () => {
  return (
    <div>
      <div className='teck'>
        <h1 className='abc'>
        About TEKATHON 3.0
        </h1><br />
        <h2 className='cde'>Internal Hackathon</h2>
        <p className='efg'>Gather with 1,000 students from around the world and industry leaders at the University of Waterloo to collaborate and bring your ideas to life in 36 hours.<br /><br />
Hack the North aims to empower students like you who are eager to learn and create by providing an environment for you to explore and build with workshops, mentorship, and hardware. Let us take care of travel, food, and the details, so that you can grow your ideas beyond a vision.</p>
<div className='buttonpos'>
<button href="/" className="menu-link-button">
              Know more
            </button></div>
      </div>
      <img src='about.png' className='aboutimg' alt='aboutimage' />
      <img src='aboutsec.png' className='aboutimgsec' alt='aboutimage' />
      <h1 className='abcd'>
      About Smart India Hackathon 2024 
        </h1><br />
        <h2 className='cdef'>Government Hackathon</h2>
        <div className='buttonposi'>    <p className='efgh'>Gather with 1,000 students from around the world and industry leaders at the University of Waterloo to collaborate and bring your ideas to life in 36 hours.<br /><br />
Hack the North aims to empower students like you who are eager to learn and create by providing an environment for you to explore and build with workshops, mentorship, and hardware. Let us take care of travel, food, and the details, so that you can grow your ideas beyond a vision.</p>
<div className='buttonpos'>
<button href="/" className="menu-link-button">
              Know more
            </button></div></div>
    </div>
  )
}

export default About
